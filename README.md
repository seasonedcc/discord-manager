...
| `mentions_list` | The messages that pinged you since a moment in time — someone naming you, plus replies to you the sender left the ping on — each with its text, the text of any embeds it carries, what is attached to it, and the reactions still on it |
...
- **Reactions are captured the same way, and only while they stand.** A message the bot records now arrives with the reactions it already had, and every reaction after that lands as it happens — but messages ingested before this version show none, for the same reason embeds do. What comes back is what Discord shows: a reaction somebody took back is simply gone, and who took it back is nobody's business but theirs.
