...
- **Replies that say what they answer** — Discord shows a reply under a header naming the message it answers, and a sender who switches the ping off does not take that header away. Every message comes back with the message it answers, so a reply that never mentioned you and never pinged you still reads as an answer rather than a remark out of nowhere.
...
The seed only ever runs against a freshly created, empty database. With demo credentials, everything that reads from the local store works — catch-ups, mentions, bookmarks, channels, the people who posted, and `ingestion_status`, which reads as a bot connected and caught up on every channel. Sending fails, and so do `messages_fetch` and `threads_create`: all three go to Discord live and need a real bot token and a real server. The seed leaves one refused send behind so you can see `messages_send_status` offer a retry.
...
| `members_list` | The people the bot has seen post in your server, each with the `discordUserId` a `<@id>` mention needs and the username and display name they go by now — matched on part of a name in any case, or taken whole |
...
| `messages_catch_up` | Everything posted since a moment in time, across the server or in one channel, 200 at a time — each message with its text, the text of any embeds it carries, what is attached to it, the reactions still on it, and, when it is a reply, which message it answers |
| `mentions_list` | The messages that pinged you or your bot since a moment in time — someone naming either of you, plus replies the sender left the ping on — each with its text, the text of any embeds it carries, what is attached to it, the reactions still on it, and, when it is a reply, which message it answers |
...
| `bookmarks_list` | The bookmarks still waiting on you, each with its reason, its text, the text of any embeds it carries, what is attached to it, the reactions still on it, and, when it is a reply, which message it answers — snoozed ones and single-reason views on request |
...
- **Attachment links are fingerprints, not archives.** Discord signs the URL of every uploaded file and stops honoring it after about a day. The store keeps the filename, the size and that URL so your assistant can say what came with a message and open it while the link is fresh — nothing is ever downloaded, so an old bookmark names its attachment without being able to fetch it. When the link has gone stale, `messages_fetch` asks Discord for that one message again and comes back with a freshly signed one.
