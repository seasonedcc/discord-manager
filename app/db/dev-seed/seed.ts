...
console.log(
  `Seeded a development server: two channels, an archived thread, six messages — one of them an alert that says everything in an embed and carries a screenshot — one mention of you, one reply that pinged you without naming you, two bookmarks — one captured with the 🔖 reaction and still sitting in Inbox, one filed under Answer later — one send Discord refused, and one live fetch of that alert. Start the MCP server with pnpm run mcp, ask your assistant to catch up on #engineering, read messages_send_status for request ${refusedSend.send.requestId} to see the guarded retry it offers, and call messages_fetch on message ${alert.messageId} to see Discord's current wording, a freshly signed attachment link and who reacted.`
)
